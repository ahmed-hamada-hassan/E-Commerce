...
internal sealed class RegisterCustomerCommandHandler : IRequestHandler<RegisterCustomerCommand, Result<AuthResponse>>
{
    ...
    public RegisterCustomerCommandHandler(UserManager<ApplicationUser> userManager, ITokenService tokenService, 
        IOptionsSnapshot<JWTSettings> jwtSettings, IFileService fileService, ILogger<RegisterCustomerCommandHandler> logger)
    {
    ...
    public async Task<Result<AuthResponse>> Handle(RegisterCustomerCommand request, CancellationToken cancellationToken)
    {
        ...
        var roleResult = await _userManager.AddToRoleAsync(userValue, AppRoles.Customer);

        if (!roleResult.Succeeded)
        {
            var firstError = roleResult.Errors.First();
            _logger.LogCritical("Failed to assign role to UserID: {UserId}. Error: {ErrorDescription}", userValue.Id, firstError.Description);
            return Result<AuthResponse>.Failure(new Error(firstError.Code, firstError.Description));
        }
